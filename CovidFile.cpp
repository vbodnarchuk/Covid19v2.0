...
int main() {
	...
	for (int f = 1; f < 1000000; f++) { //Ïåðøà ñòàä³ÿ íàâ÷àííÿ 90% òî÷íîñò³ äîïîì³æíà
		w11 = 2 + rand() %2000;
		q = second_day_correct[st] - first_day_correct[st];
		correct_init1 = second_day_correct[st] + q + q * Nnasel / w11;//Íåéðîí
	...
	st = 0;
	w1 = w1 + w11 / 2;
	for (int b = 1; b < 1000000; b++) { //Äðóãà ñòàä³ÿ íàâ÷àííÿ 95% òî÷íîñò³
		w2 = 2 + rand() % 2000;
		w3 = w1 + w2 / 2;
		q = second_day_correct[st] - first_day_correct[st];
		correct_init = second_day_correct[st] + q+ q* Nnasel/w3;//Íåéðîí
		if (correct_init < third_day_correct[st]) {
	...
	for (int h = 1; h < 1000000; h++) { //Äðóãà ñòàä³ÿ íàâ÷àííÿ 95% òî÷íîñò³ äîïîì³æíà
		w21 = 1 + rand() % 2000;
		w31 = w1 + w2 / 2;
		q = second_day_correct[st] - first_day_correct[st];
		correct_init2 = second_day_correct[st] + q + q * Nnasel / w31;//Íåéðîí
	...
	//-------------------------------------------------Êîðåêòèðîâêà------------------------------------
	for (int x = 1; x < 1000000; x++) { //Êîðåêòèðîâêà ç áàçîþ äàííèõ
		w8 = 1 + rand() % 2000;
		w9 = w7 + w8 / 2;
		q = second_day_correct[st] - first_day_correct[st];
		correct_init4 = second_day_correct[st] + q + q * Nnasel / w9;//Íåéðîí
	...
	for (int xx = 1; xx < 1000000; xx++) { //Êîðåêòèðîâêà ç áàçîþ äàííèõ äîïîì³æíà
		w81 = 1 + rand() % 2000;
		w91 = w7 + w81 / 2;
		w911 = w91 + w911 / 2;
		q = second_day_correct[st] - first_day_correct[st];
		correct_init3 = second_day_correct[st] + q + q * Nnasel / w911;//Íåéðîí
	...
	for (int a = 1; a <= d; a++) {
		...
		Sleep(200);
		if (N2 >= 7780000000) {
			cout << ("All people will be infected after ") << a + 1 << (" days") << endl;//Âåñü ñâ³ò çàðàæåíèé
			break;
		}
		if (N1 >= N2) {
			cout << ("Error...") << endl;// Ïîìèëêà
			break;
		}
	}
	cout << ("Please stay home") << endl;
